...
AddEventHandler("scf_telegram:check_inbox", function()
    ...
    getOrGenerate(Character.citizenid, function(postbox)
        result = MySQL.Sync.fetchAll('SELECT * FROM telegrams WHERE recipient = @reci ORDER BY id DESC', { ['@reci'] = postbox }) --, function(result)
			local res = {}
            res['box'] = postbox
...
AddEventHandler("scf_telegram:check_inboxPD", function()
    local _source = source
	result = MySQL.Sync.fetchAll('SELECT * FROM telegrams WHERE recipient = @reci ORDER BY id DESC', { ['@reci'] = "anon" })--, function(result)
		local res = {}
		res['box'] = "ANON"
		res['firstname'] = "ANON"			
		res['list'] = result
            ...
            result = MySQL.Sync.fetchAll('SELECT postbox FROM players WHERE postbox = @pox',{['@pox'] = data.recipient}) --, function(result)
                if result[1] ~= nil then
                    if data.recipient == nil or data.recipient == '' and data.subject == nil or data.subject == '' then
                        ...
                        local Parameters = { ['recipient'] = data.recipient, ['sender'] = data.sender, ['subject'] = data.subject, ['sentTime'] = sentDate, ['message'] = data.message, ['postoffice'] = data.postoffice }
...
AddEventHandler("scf_telegram:getTelegram", function(tid)
    ...
    local telegram = {}
    Citizen.Wait(0)
    result = MySQL.Sync.fetchAll('SELECT * FROM telegrams WHERE id = @id', { ['@id'] = tid }) --, function(result)
